...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-14 items-center justify-between px-4 sm:px-6">
          ...
          <div className="w-full text-left sm:w-auto">
            <h1 className="text-md sm:text-lg font-semibold tracking-tight">Product-design.guide</h1>
            <span className="text-sm text-muted-foreground hidden sm:inline">for New Founders</span>
          </div>
      ...
      <main className="container py-6 md:py-16 px-4 sm:px-6">
        <div className="text-center space-y-4 mb-10 md:mb-24">
          <h1 className="text-2xl md:text-4xl font-bold tracking-tight">Product Design Process</h1>
          <div className="md:hidden w-32 mx-auto mb-4">
            <Progress value={progress} className="h-2" />
            <p className="text-xs text-muted-foreground mt-1 text-center">{Math.round(progress)}% Complete</p>
          </div>
          ...
          {chapters.map((chapter) => {
            ...
            return (
              <Link href={`/guide?chapter=${chapter.id}`} key={chapter.id}>
                <div
                  className={`group relative border rounded-lg p-6 transition-colors ${isCompleted ? "border-primary/50 bg-primary/5" : "hover:border-primary"
                    }`}
                >
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-none">
                      <div
                        className={`flex h-8 w-8 items-center justify-center rounded-md text-sm font-medium ${isCompleted ? "bg-primary text-primary-foreground group-hover:bg-blue-700" : "bg-muted group-hover:bg-muted/80 dark:group-hover:bg-muted/60"
                          }`}
                      >
                        {chapter.number}
                      </div>
